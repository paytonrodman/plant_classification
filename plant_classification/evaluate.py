...
def main(
    ...
    test_data  = compile_data.create_generator(INTERIM_DATA_DIR / "test", image_shape, num_classes, batch_size)
    valid_data = compile_data.create_generator(INTERIM_DATA_DIR / "valid", image_shape, num_classes, batch_size)

    logger.info("Evaluating model...")
    score = make_statistics.get_basic_stats(model, test_data, batch_size)
    print('Test loss:', score[0])
    print('Test accuracy:', score[1], '\n')

    report = make_statistics.get_report(model, test_data)
    print(report, '\n')
    ...
    make_figures.plot_confusion(model, test_data, save_path)
